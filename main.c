...
void main(void)
{
    uint8_t i = 0;
    uint8_t result = 0;
    //uint8_t msbT1, lsbT1, msbT2, lsbT2, msbT3, lsbT3;
    ...
    i2cWriteSequence(0xEC, 0xF4, 0xFF); //write 0x2f to ctrl_meas register 0xF4 which sets up the following conditions: osrs_t = 001 , osrs_p =011 , mode 11 = 0x2f
    i2cWriteSequence(0xEC, 0xF5, 0x00); //write 0x10 to config register 0xF5 which sets up the following conditions:  t_sb = 000, filer 100, spi3w_en = 0x10
    
    T1 = i2cMultipleReadTwo(0x88);
    printf("T1 %u\n", T1);
